...
// Alt S-box
void aes_round_alt_sbox(uint8_t block[AES_BLOCK_SIZE], uint8_t round_key[AES_BLOCK_SIZE], int lastround)
{
	int i;
	uint8_t tmp;
	 ...
	 */
	/* Row 0 */
	block[ 0] = S_alt[block[ 0]];
	block[ 4] = S_alt[block[ 4]];
	block[ 8] = S_alt[block[ 8]];
	block[12] = S_alt[block[12]];
	/* Row 1 */
	tmp = block[1];
	block[ 1] = S_alt[block[ 5]];
	block[ 5] = S_alt[block[ 9]];
	block[ 9] = S_alt[block[13]];
	block[13] = S_alt[tmp];
	/* Row 2 */
	tmp = block[2];
	block[ 2] = S_alt[block[10]];
	block[10] = S_alt[tmp];
	tmp = block[6];
	block[ 6] = S_alt[block[14]];
	block[14] = S_alt[tmp];
	/* Row 3 */
	tmp = block[15];
	block[15] = S_alt[block[11]];
	block[11] = S_alt[block[ 7]];
	block[ 7] = S_alt[block[ 3]];
	block[ 3] = S_alt[tmp];
...
// alternate S-box
void aes128_enc_alt_sbox(uint8_t block[AES_BLOCK_SIZE], const uint8_t key[AES_128_KEY_SIZE], unsigned nrounds, int lastfull)
{
	uint8_t ekey[32];
	int i, pk, nk;
	...
	for (i = 1; i < nrounds; i++)
	{
		aes_round_alt_sbox(block, ekey + nk, 0);
		pk = (pk + 16) & 0x10;
		nk = (nk + 16) & 0x10;
		next_aes128_round_key(ekey + pk, ekey + nk, i);
	}
	...
	{
		aes_round_alt_sbox(block, ekey + nk, 16);
	}
}
